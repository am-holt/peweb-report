...
\lstdefinelanguage{MWE2}{
  keywords={module, Workflow, component, var},
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
  morestring=[b]',
  morestring=[b]",
...
\lstdefinelanguage{Editor}{
  keywords={Node, Projection, view, get, set, validate,Component},
  morecomment=[l]{//},
  morestring=[s]{<<}{>>},
  keywordstyle=\color{blue}\bfseries,
  ndkeywordstyle=\color{darkgray}\bfseries,
...
\lstdefinelanguage{Xtend}{
  keywords={import,package,static,extends,class,void,val,def, Workflow, component},
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
  morestring=[b]{'''},
  morestring=[b]",
...
\lstdefinelanguage{Xtext}{
  keywords={grammar,import,hidden,generate},
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
  morestring=[b]',
  morestring=[b]",
...
%The API and DSL are developed into an extension of the Xtext language engineering workbench. In the extension, an arbitrary language grammar defined in Xtext is translated into a web application that uses the API and custom projections. This creates a web-based projectional editor for a given language with very little additional effort from the language designer, which make programming with that DSL more widely accessible.
\\
\\
...
% both in terms of its flexibility and ease-of-use to a language designer creating a projectional editor, and the impact the resultant editors may have on the accessibility of DSLs to domain experts with little or no experience of coding. 
...
%In Section \ref{technicalBackground} I begin by discussing the necessary technical background. Across Sections \ref{generation} to \ref{clientApp} I then discuss the design and implementation of my Xtext extension, each section focussing on a different aspect of the tool. In Section \ref{evaluation} I evaluate the final tool's effectiveness, before concluding with a critical review of the requirements from \ref{requirements} and then outlining possible ventures for future work in Section \ref{conclusion}. 
...
\lstinputlisting[language=MWE2, tabsize=2 ]{Listings/GenerateMyDsl.mwe2}
...
%I now discuss the design of the API itself, giving its full specification in the appendix. When designing this API it was necessary to consider the relevant requirements for the project's ultimate goal as given in \ref{requirements}. The requirements to produce a lightweight (\RLightweight) and customisable (\RCustom) editor are clearly relevant here.
%\\
%\\
...
\lstinputlisting[language=Editor,showstringspaces=false, tabsize=2]{Listings/greetingsProjections.editor}
\begin{figure}[h!]
  \centering
  \begin{subfigure}[b]{0.45\linewidth}
...
\lstinputlisting[language=Editor,showstringspaces=false,tabsize=2]{Listings/TextBox.editor}
...
%send information about a node's state to a client using the get-node API call as was touched upon in \ref{apiNodeStateTransfer}. 
%
...
Evaluation of the project uncovered room for future investigation. As mentioned in previous sections, obvious candidates are:
\begin{itemize}
...
\item Conducting evaluation on the performance of the projectional editors produced in comparison to their textual counterparts, perhaps by designing a range of languages and measuring performance on files of various sizes.
